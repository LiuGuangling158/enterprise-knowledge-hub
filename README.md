...
- 文档新建、编辑、版本记录、服务端上传解析、原始文件落盘、协作评论、归档恢复、提交审批、审批通过/驳回
- 文档列表、详情、分块混合检索、审批列表、文档级审批轨迹 API
- Query 改写、BM25 关键词召回、本地语义召回、元数据过滤、RRF 融合排序和引用输出
- SSE 流式问答输出
- 问答对话历史持久化与用户级隔离
- RAG 引用来源结构化输出，包含文档、版本、章节、片段和分数
- 文档创建/更新时自动生成摘要
- 文档版本差异比较，支持增删行统计和 diff 展示
- Agent 工作台，展示 Router、Retrieval、QA、Memory 队列和工具能力
- Agent 可观测执行链路，问答时输出计划、工具调用、证据校验、记忆保存和原始事件
- 多租户与部门隔离字段设计
- 管理员后台，支持用户、部门、文档、审批、上传记录和关键指标查看，并可调整用户角色/部门
- 审核 Agent 已接入提交审批流程，输出格式、重复内容和敏感信息风险，审批中心可查看
